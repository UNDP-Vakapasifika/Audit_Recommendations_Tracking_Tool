...
{   
    ...
    {   
        ...
        $filter_audit_type = $selectedFilters['filter_audit_type'] ?? [];
        $filter_risk_level = $selectedFilters['filter_risk_level'] ?? 'all';
        ...
        if (!empty($filter_audit_report_title)) {
            $query->whereIn('audit_report_title', (array) $filter_audit_report_title);
        }
        ...
        if (!empty($filter_recurrence)) {
            $query->whereIn('recurrence', (array) $filter_recurrence);
        }
        if (!empty($filter_audit_type)) {
            $query->whereIn('audit_type', (array) $filter_audit_type);
        }
        if ($filter_risk_level !== 'all') {
            $query->where('classification', $filter_risk_level);
        }

        // Handle filters for related models
        if (!empty($selectedFilters['filter_client_types'])) {
            $query->whereHas('leadBody', function ($q) use ($selectedFilters) {
                $q->whereIn('client_type_id', $selectedFilters['filter_client_types']);
            });
        }

        if (!empty($selectedFilters['filter_mainstream_gender'])) {
            $mainstreamGenderFilters = (array) $selectedFilters['filter_mainstream_gender'];
            $query->whereHas('mainstreamCategory', function ($q) use ($mainstreamGenderFilters) {
                $q->whereIn('name', $mainstreamGenderFilters);
            });
        }
        ...
        $totalRecurrence = FinalReport::distinct()->where('recurrence', 'Yes')->count();
