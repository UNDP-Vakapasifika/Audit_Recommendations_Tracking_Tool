            ...
            <div class="px-6 py-4">
                <div class="flex flex-wrap gap-4 md:gap-6">
                    <div class="flex flex-col w-fit">
                        <span class="text-gray-700 font-bold">Country Office</span>
                        <span class="text-gray-700">{{ $finalReport->country_office }}</span>
                    </div>
                    ...
                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                        <span class="text-gray-700 font-bold">Repeated Finding</span>
                        <span class="text-gray-600">{{ $finalReport->recurence }}</span>
                    </div>
                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                    ...
                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                        <span class="text-gray-700 font-bold">Target Completion Date</span>
                        <span class="text-gray-600">{{ $finalReport->target_completion_date->format('d, M Y') }}</span>
                    </div>
                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                    ...
                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                        <span class="text-gray-700 font-bold">Page Par Ref.</span>
                        <span class="text-gray-600">{{ $finalReport->page_par_reference }}</span>
                    </div>
                    ...
                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                        <span class="text-gray-700 font-bold">Client</span>
                        <span class="text-gray-600">{{ $finalReport->responsible_entity }}</span>
                    </div>

                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                        <span class="text-gray-700 font-bold">Sai Confirmation</span>
                        <span class="text-gray-600">{{ $finalReport->sai_confirmation }}</span>
                    </div>

                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                        <span class="text-gray-700 font-bold">Head of Audited Entity</span>
                        <span class="text-gray-600">{{ $finalReport->head_of_audited_entity }}</span>
                    </div>

                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                        <span class="text-gray-700 font-bold">Audited Entity Focal Point</span>
                        <span class="text-gray-600">{{ $finalReport->audited_entity_focal_point }}</span>
                    </div>
                    ...
                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                        <span class="text-gray-700 font-bold">Summary of Response</span>
                        <span class="text-gray-600">{{ $finalReport->summary_of_response }}</span>
                    </div>

                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                        <span class="text-gray-700 font-bold">Classification</span>
                        <span class="text-gray-600">{{ $finalReport->classfication }}</span>
                    </div>

                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                        <span class="text-gray-700 font-bold">Responsible Person</span>
                        <span class="text-gray-600">{{ $finalReport->responsible_person }}</span>
                    </div>

                    <div class="flex flex-col mt-2 md:mt-0 w-fit">
                        <span class="text-gray-700 font-bold">Responsible Person Email</span>
                        <span class="text-gray-600">{{ $finalReport->responsible_person_email }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="mt-4" id="caution">
                    <div style="overflow-x: auto;">
                        <table class="w-full border border-gray-200"
                            ...
                            <tbody>
                                @forelse($finalReport->cautions as $caution)
                                    <tr>
                                        <td class="border border-gray-200 px-4 py-2">{{ $caution->user->name }}</td>

                                        <td class="border border-gray-200 px-4 py-2">
                                            {{ $caution->created_at->format('d, M Y') }}</td>
                                        <td class="border border-gray-200 px-4 py-2">{{ $caution->message }}</td>
                                        <td class="border border-gray-200 px-4 py-2">
                                            @if (auth()->user()->can('delete final report cautions'))
                                                <form class="delete_item"
                                                    action="{{ route('final-report.caution.destroy', $caution->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td class="border border-gray-200 px-4 py-2" colspan="4">No Cautions</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
        ...
        <div x-show="showModal" class="fixed inset-0 overflow-y-auto">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showModal"
                    ...
                    <form action="{{ route('final-report.caution.store', $finalReport->id) }}" method="post">
                        @csrf
                        <div class="w-full px-4 pt-4">
                            <x-input-label for="message" :value="__('Caution Message')" />
                            <x-textarea-input id="message" class="block mt-1 w-full" type="text" name="message"
