...
@section('body')
    <div class="flex flex-col md:flex-row justify-start md:justify-between md:items-center">
        <h3 class="page-heading">Mainstream Categories</h3>
        @if (auth()->user()->can('create lead bodies'))
            <a href="{{ route('mainstream_category.create') }}"
                class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                Add Mainstream Category
            </a>
        @endif
    </div>
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="p-4">
                        <table class="w-full border border-gray-200 mt-3"
                            ...
                            <thead>
                                <tr>
                                    <th class="border border-gray-200 px-4 py-2">Category Name</th>
                            ...
                            <tbody>
                                @foreach ($mainstream_categories as $category)
                                    <tr>
                                        ...
                                        <td class="border border-gray-200 px-4 py-2 flex gap-3">
                                            @if (auth()->user()->can('edit lead bodies'))
                                                <a href="{{ route('mainstream_category.edit', $category->id) }}" class="text-blue-500 hover:underline">
                                            ...
                                            @if (auth()->user()->can('delete lead bodies'))
                                                <form class="delete_item"
                                                    action="{{ route('mainstream_category.destroy', $category->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
