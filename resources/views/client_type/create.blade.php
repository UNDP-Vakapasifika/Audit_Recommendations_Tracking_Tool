...
@section('body')
    <h3 class="page-heading">
        {{ isset($lead_body) ? 'Edit Client' : 'Add New Client Type' }}
    </h3>
    ...
    <div class="py-4">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4">
            ...
            <form method="POST" action="{{ isset($lead_body) ? route('client-types.update', $lead_body->id) : route('client-types.store') }}">
                @csrf
                @if(isset($lead_body))
                    @method('PUT')
                @endif
